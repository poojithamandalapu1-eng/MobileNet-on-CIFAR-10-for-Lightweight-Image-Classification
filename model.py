...
"""
print("Buliding MobileNet model...")
...
model = models.Sequential([
    ...
    layers.Droput(0.3),
    layers.Dense(128, actiavtion='relu'),
    layers.Dropout(0.2),
...
def unfreeze_model(model, base_model, unfreeze_layers=30);
    ...
    for layer in base_model.layer[:-unfreezze_layers]:
        layer.trainable = False

        print("Model ready for fine-tuning!")
        return model
    
    def get_modelsize(model):
        """
        Print model parameter count and size.
        """
        total_params = model.count_params()
        size_md = (total_params * 4) / (1024 * 1024)
